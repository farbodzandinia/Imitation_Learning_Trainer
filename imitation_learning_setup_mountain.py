...
class DataCollectorMountain:
    ...
    def __init__(self, save_directory):
        ...
        rospy.loginfo("DataCollectorMountain initialized.")
    ...
    def shutdown_hook(self):
        self.csv_file.close()
        rospy.loginfo("DataCollectorMountain shutdown, saved {} images.".format(self.img_count))

if __name__ == "__main__":
    
    rospy.init_node('data_collector_mountain_node', anonymous=True)
    save_dir = rospy.get_param('~save_dir', 'data_mountain')
    collector = DataCollectorMountain(save_dir)
    rospy.on_shutdown(collector.shutdown_hook)

    try:
        rospy.spin()
    except KeyboardInterrupt:
        rospy.loginfo("Shutting down DataCollectorMountain node.")
