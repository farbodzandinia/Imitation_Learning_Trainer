...
class DataCollectorAsphalt:
    ...
    def __init__(self, save_directory):
        ...
        rospy.loginfo("DataCollectorAsphalt initialized.")
    ...
    def shutdown_hook(self):
        self.csv_file.close()
        rospy.loginfo("DataCollectorAsphalt shutdown, saved {} images.".format(self.img_count))

if __name__ == "__main__":
    
    rospy.init_node('data_collector_asphalt_node', anonymous=True)
    save_dir = rospy.get_param('~save_dir', 'data_asphalt')
    collector = DataCollectorAsphalt(save_dir)
    rospy.on_shutdown(collector.shutdown_hook)

    try:
        rospy.spin()
    except KeyboardInterrupt:
        rospy.loginfo("Shutting down DataCollectorAsphalt node.")
